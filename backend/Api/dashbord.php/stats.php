...
try {
    ...
    // Statistiques des crédits (utilisant le modèle Credit)
    ...
    $stats = [
        ...
        'creditsTotalEncours' => (float)($creditStats['montant_restant_dû'] ?? 0), // Montant restant dû (vraies données)
        'creditsNombre' => (int)($creditStats['credits_impayés'] ?? 0), // Nombre de crédits impayés (vraies données)
        'creditsRecouvrement' => (float)($creditStats['montant_restant_dû'] ?? 0), // Total à recouvrer (vraies données)
